...
int split(char *str, const char *del)
{
	...
	char *time;
	char *date;
	void setDate(time_t timep);
	s=strtok(str,del);
	if(strcmp("$GPRMC",s)==0){
		time = strtok(NULL,del);
		strtok(NULL,del);
		strtok(NULL,del);
		strtok(NULL,del);
		strtok(NULL,del);
		...
		strtok(NULL,del);
		strtok(NULL,del);
		date = strtok(NULL,del);
		time_t timep=DateStrToSec(date,time);
		setDate(timep);
		return 0;
	}
	else
...
//此方法链接gps时可能需要修改包括时区和输出当前时间
void setDate(time_t timep){
	struct timeval tv;
	tv.tv_sec = (long)timep;
	settimeofday(&tv,NULL);//修改系统时间，用的时候取消注释
	int sec = timep;
	printf("%d\n",sec);
	time(&timep);//当前系统时间，与settimeofday一起取消注释
